...
if (!empty($_POST)) {
    ...
    $checkIn->name = $_POST["userName"];        //key names come from name attributes on form input fields
    $checkIn->rating = $_POST["rating"];
    $checkIn->review = $_POST["review"];
    $checkIn->timestamp = date("Y-m-d", time());

    $serialisedCheckIn = serialize($checkIn);
    file_put_contents("checkins.txt", $serialisedCheckIn, FILE_APPEND);
}
